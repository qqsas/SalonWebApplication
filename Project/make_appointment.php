...
foreach ($dates as $date) {
    $stmt = $conn->prepare("SELECT Time, Duration FROM Appointment WHERE BarberID = ? AND DATE(Time) = ? AND Status != 'Cancelled'");
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $appointments[$date][] = [$start, $start + $row['Duration']*60];
    }
}
...
.week-nav { margin-top: 20px; }
.week-nav a { margin: 0 10px; text-decoration: none; color: #007bff; }
</style>
<script>
    ...
    <tr>
        ...
        <?php foreach ($dates as $date): 
            ...
            foreach ($working_hours[$date] as $wh) {
                if ($slot_start >= $wh['start'] && $slot_end <= $wh['end']) { $in_working = true; break; }
            }
            if (!$in_working) $class = 'unavailable';

            // Check booked appointments
            foreach ($appointments[$date] as $app) {
                if ($slot_start < $app[1] && $slot_end > $app[0]) { $class = 'unavailable'; break; }
            }

            // Check unavailability
            foreach ($unavailable[$date] as $ua) {
                if ($slot_start < $ua[1] && $slot_end > $ua[0]) { $class = 'unavailable'; break; }
            }

            // Past times
            if ($slot_start < time()) $class = 'unavailable';

        ?>
        <td class="<?= $class ?>" onclick="selectSlot(this,'<?= date('Y-m-d H:i:s', $slot_start) ?>')"></td>
        <?php endforeach; ?>
    </tr>
    <?php endforeach; ?>
</table>
